...
//        }
    ...
    public function change_password($identity, $old, $new)
    {
        ...
        if ($old_password_matches === true) {
            ...
            $successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data,
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array())
    {
        ...
        $data = array(
            ...
            'status' => ($manual_activation === false ? true : false)
        );
...
//        }
    ...
    public function login($identity, $password, $remember = false)
    {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, status, updated_at')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
...
//            $password = $this->hash_password_db($user->id, $password);
            $password = $this->verify_password($password, $user->password);
    ...
    {
        return $db_password==md5($password);
    }
        ...
        if ($recheck !== 0) {
            ...
            if ($last_login + $recheck < time()) {
                $query = $this->db->select('id')
                    ->where(array($this->identity_column => $this->session->userdata('identity'), 'status' => true))
                    ->limit(1)
                    ->order_by('id', 'desc')
                    ->get($this->tables['users']);
                if ($query->num_rows() === 1) {
    ...
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $data = array('ip_address' => '', 'email' => $identity, 'time' => time());
            if ($this->config->item('track_login_ip_address', 'ion_auth')) {
    ...
    public function remove_from_group($group_ids = false, $user_id = false)
    {
        ...
        if (!empty($group_ids)) {
            ...
            // Cast to float to support bigint data type
            if ($return = $this->db->delete($this->tables['users_groups'],
                array($this->join['users'] => (float)$user_id))) {
    ...
    public function update_last_login($id)
    {
        ...
        $this->db->update($this->tables['users'], array('updated_at' => date("Y-m-d h:i:sa")), array('id' => $id));
    ...
    public function set_session($user)
    {
        ...
        $session_data = array(
            ...
            'old_last_login' => $user->updated_at,
            'last_check' => time(),
        );
